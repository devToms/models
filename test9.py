...
predictions = []
with open('/home/tomasz/projekty/python/app_market_bot/app_market_bot/model_manager/neural_network_models/training_logs/river_training_v1_log.txt', 'r') as f:
    for line in f:
        if any(x in line for x in ['River-V....-DECISION']):
...
# Zapis wyników
combined.to_csv('predictions_evaluation_detailed_v1.csv', index=False)
with open('prediction_performance_report_v...txt', 'a') as f:
    f.write(report)
