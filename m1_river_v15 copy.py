...
training_log_file = "/home/tomasz/projekty/python/app_market_bot/app_market_bot/model_manager/neural_network_models/training_logs/river_training_v15_log.txt"
training_logger = LightLogger(training_log_file, logger_name="training_logger")

short_term_predictor = "/home/tomasz/projekty/python/app_market_bot/app_market_bot/model_manager/neural_network_models/training_logs/short_term_predictor_v1_log.txt"
short_term_predictor_logger = LightLogger(short_term_predictor, logger_name="short_term_predictor_logger")


training_log_file = "/home/tomasz/projekty/python/app_market_bot/app_market_bot/model_manager/neural_network_models/training_logs/river_training_v15_api_log.txt"
training_api_logger = LightLogger(training_log_file, logger_name="training_api_logger")


training_log_file = "/home/tomasz/projekty/python/app_market_bot/app_market_bot/model_manager/neural_network_models/training_logs/river_training_v15_order_log.txt"
training_order_logger = LightLogger(training_log_file, logger_name="training_order_logger")


training_log_file = "/home/tomasz/projekty/python/app_market_bot/app_market_bot/model_manager/neural_network_models/training_logs/river_training_v15_account_log.txt"
training_account_logger = LightLogger(training_log_file, logger_name="training_account_logger")


training_log_file = "/home/tomasz/projekty/python/app_market_bot/app_market_bot/model_manager/neural_network_models/training_logs/river_training_v15_websocket_log.txt"
...
class TradingSignal:
    ...
    def __init__(self, action: str, confidence: float, bid: float, 
                 ...
                 short_term_pred: Optional[float] = None):
    ...
    def get_recommended_entry(self) -> float:
        """Zwraca sugerowany poziom wejścia, uwzględniając przewidywaną korektę"""
        if self.short_term_pred is None:
            return self.bid
        return self.bid + (self.short_term_pred * 0.5)  # Agresywność wejścia
...
class OrderExecutor:
    ...
    def __init__(self, api_client: ApiOrderClient, trade_direction: str = 'sell'):
        self.api = api_client
    ...
    async def execute(self, signal: TradingSignal) -> bool:
        ...
        try:
            ...
            if hasattr(signal, 'short_term_pred') and signal.short_term_pred is not None:
                if signal.short_term_pred < -0.0005:  # Jeśli przewidywany spadek > 0.5 pipsa
                    self.logger.info(f"🔄 Oczekiwanie na korektę: przewidywany spadek {signal.short_term_pred*10000:.1f} pips")
                    await asyncio.sleep(2)  # Czekaj 2 sekundy
                    return False
...
class ShortTermPredictor:
    def __init__(self, window_size: int = 5,model_save_path: str = "/home/tomasz/projekty/python/app_market_bot/app_market_bot/model_manager/neural_network_models/river_models/river_model_short_term_v1.pkl.gz"):
        self.logger = short_term_predictor_logger
    ...
    def update(self, bid: float, ask: float) -> Optional[float]:
        try:
            self.window.append({"bid": bid, "ask": ask})
            
            if len(self.window) < self.window.maxlen:
                return None
            
            # Przygotuj dane - konwertuj na listę i upewnij się, że to pojedyncze wartości
            diffs = [tick["bid"] for tick in self.window]
            X = float(np.diff(diffs)[-1])  # Bierzemy ostatnią różnicę jako float
            y = float(bid - self.window[-1]["bid"])  # Konwersja na float
            
            # Ucz model
            self.model.learn_one(X, y)
            
            # Przewidź następną zmianę
            next_change = float(self.model.predict_one(X))
            
            # Aktualizuj metryki
            true_change = float(bid - self.window[-1]["bid"])
            self.mae.update(true_change, next_change)
            self.accuracy.update(true_change > 1e-6, next_change > 1e-6)
            
            return next_change
        
        except Exception as e:
            self.logger.error(f"ShortTermPredictor update error: {str(e)}")
            return None
    ...
    def save_model(self) -> None:
        ...
        try:
            ...
            backup_path = self.model_save_path.parent / f"river_model_short_term_v1_{timestamp}.pkl.gz"
    ...
    def _load_or_init_model(self) -> compose.Pipeline:
        if self.model_save_path.exists():
            try:
                model = pickle.load(gzip.open(self.model_save_path, "rb"))
                self.mae = metrics.MAE()  # Reset metryk
                self.accuracy = metrics.Accuracy()
                return model
            except Exception as e:
...
class RiverModelTrainer:
    ...
    def __init__(self, model_save_path: str = "/home/tomasz/projekty/python/app_market_bot/app_market_bot/model_manager/neural_network_models/river_models/river_model_v15.pkl.gz"):
        ...
        self.short_term_predictor = ShortTermPredictor()
        self.last_short_term_pred = None
    ...
    def process_tick(self, tick_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            short_term_pred = self.short_term_predictor.update(tick["bid"], tick["ask"])
            self.last_short_term_pred = short_term_pred
            ...
            return {
                ...
                **signal,
                ...
                "volatility": X['volatility'],
                "ema_ratio": X.get('ema_ratio', 1.0),
                "short_term_pred": short_term_pred,
                "recommended_entry": tick["bid"] + (short_term_pred * 0.3) if short_term_pred else tick["bid"]  # DODAJ TO
            }
    ...
    def save_model(self) -> None:
        ...
        try:
            ...
            # Create timestamped backup
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            backup_path = self.model_save_path.parent / f"river_model_v15_{timestamp}.pkl.gz"
            with gzip.open(backup_path, "wb") as f:
                pickle.dump(self.model, f)
    ...
    while True:
        try:
            async with websockets.connect(uri, ping_interval=20, ping_timeout=10) as websocket:
                logger.info("🚀 River V15 connected to websocket")
                logger.info(f"💡 Strategy: {executor.trade_direction.upper()} only")
                ...
                while True:
                    try:
                        ...
                        signal = TradingSignal(
                            ...
                            short_term_pred=decision.get('short_term_pred')
                        )
                        ...
                        logger.info(f"   Pips: {signal.pips:.1f} | Conf: {signal.confidence:.2f}{warning}")
