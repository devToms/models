...
class OrderExecutor:
    ...
    def __init__(self, api_client: ApiOrderClient, trade_direction: str = 'both'):
        self.api = api_client
        ...
        self.max_eur_loss = 2.00  # Maksymalna strata w EUR
    ...
    async def _continuous_monitor(self):
        """Wzmocniony monitoring pozycji z dokładnym debugowaniem"""
        while True:
            try:
                # 1. Wymuś aktualizację danych
                await self.account.refresh_data()
                
                # 2. Pobierz dane na dwa sposoby dla porównania
                account_data = self.account.get_account()
                position_data = await self.account.get_open_position()
                
                # 3. Debugowanie - wypisz pełne dane
                # print(f"\n[DEBUG] Full account data: {account_data}")
                # print(f"[DEBUG] Full position data: {position_data}")
                
                # 4. Sprawdź profit
                if position_data:  # Jeśli pozycja istnieje
                    profit = float(position_data.get('profit', 0))
                    # print(f"[POSITION] Current profit: {profit:.2f} EUR (from position data)")
                    
                    if profit <= -self.max_eur_loss:
                        # print(f"🛑 EMERGENCY CLOSE at {profit:.2f} EUR loss!")
                        await self._close_position()
                # else:
                    # print("[MONITOR] No open position detected")
            ...
            await asyncio.sleep(1)  # Sprawdzaj co 1 sekundę
...
class RiverModelTrainer:
    ...
    def __init__(self, model_save_path: str = "/home/tomasz/projekty/python/app_market_bot/app_market_bot/model_manager/neural_network_models/snapshots/river_model_v7_4.pkl.gz"):
...
async def river_websocket_client():
    ...
    executor = OrderExecutor(api_client, trade_direction='both')
